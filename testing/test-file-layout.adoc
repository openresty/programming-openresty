...
Test files usually has a common file extension, `.t`, to distinguish themselves
...
after this line is treated as *data* in plain text that is manipulatable
...
NOTE: The sepcial `{special_data_line}` notation is a powerful feature
of the Perl programming language that allows embedding arbitrary free-text
data in any Perl script files that are manipulatable by the containing
...
test suites. Such prologues usually define some special envirnoment variables
...
defined as testing code running upon or insode NGINX. Use of a DSL to present
...
usually contain just alphanumeric letters and underscore charaters.
...
If no visible values come after the seciton name in either form, then the
...
code to statisfy her more complicated testing requirements.
...
The original value of the `response_body` seciton above is a Perl expression
...
as the test files based on other test frameworks drived from `Test::Base`,
...
All the `Test::Base` direvatives share the same basic layout and syntax.
