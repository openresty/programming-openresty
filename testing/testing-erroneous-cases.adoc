...
unit test framework since most of the network traffic initiated by the test
...
own, listening at the port of 5678 on the local host. This is a mocked-up server
...
Earlier in this section we have already seen examples that uses the link:https://github.com/openresty/stream-lua-nginx-module#readme[ngx_stream_lua]
...
Our mocked-up Memcached server can behave in any way that we like. Yay!
